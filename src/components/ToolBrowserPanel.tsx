...
export function ToolBrowserPanel({ target, onTargetChange, onClose, onResizeStart }: ToolBrowserPanelProps) {
  ...
  const closingNativeViewRef = useRef(false);
  ...
  useEffect(() => {
    ...
    setActiveTabId(null);
    setAddressDraft("");
    closingNativeViewRef.current = false;
  }, [target]);
  ...
  useEffect(() => {
    ...
    const syncBounds = () => {
      ...
      if (!bounds) return;
      void setEmbeddedToolBrowserBounds(bounds);
    };
    ...
    return () => {
      ...
      if (!closingNativeViewRef.current) {
        void closeEmbeddedToolBrowser();
      }
    };
  }, [activeTarget, isNativeEmbedded, measureBounds]);
  ...
  }, [activeTarget, displayMode, isNativeEmbedded, measureBounds, tabs.length]);
  ...
  const closePanel = () => {
    closingNativeViewRef.current = true;
    if (isNativeEmbedded) {
      void closeEmbeddedToolBrowser()
        .catch((error) => {
