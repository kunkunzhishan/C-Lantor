...
use crate::{
    events::notify_supervisor_wake,
    prompts::{ensure_agent_workspace, event_ingest::memory_event_ingest_task},
...
async fn enqueue_event_ingest_work_item(
    ...
    let context = memory_event_ingest_task(
        &Utc::now().to_rfc3339(),
        &agent_id.to_string(),
        &event_memory_relative_dir(),
        inputs,
    );
